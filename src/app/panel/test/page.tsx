...
const buildV2Snippet = (site: TestSite, host: string) => {
  ...
  return `<script>!function(){var w=${JSON.stringify(site.websiteId)},h=${JSON.stringify(protocolLess)},s=${JSON.stringify(site.domainSlug)},v=${JSON.stringify(String(site.scriptVersion))},c=${JSON.stringify(collector)},d=${JSON.stringify(site.domain)},a=${JSON.stringify(site.allowedDomains)},p=${JSON.stringify(site.publishCode)},n=${JSON.stringify(site.collectorNode)},loaded="__elmasTestV2Loaded",fallback="__elmasTestV2FallbackSent";window[loaded]=window[loaded]||{};var script=document.createElement("script");script.src=h+"/test-cdn-v2/t-"+(typeof self!=="undefined"&&self.crypto&&typeof self.crypto.randomUUID==="function"?"1-":typeof Promise!=="undefined"?"2-":"3-")+s+"-"+v+".js";document.head.appendChild(script);function norm(e){e=String(e||"").trim();if(!e)return"";try{e=new URL(e.indexOf("://")>-1?e:"https://"+e).hostname}catch(t){var i=e.indexOf("://");e=(i>-1?e.slice(i+3):e).split(/[/?#]/)[0]}return String(e||"").replace(/:\\d+$/,"").replace(/^www\\./,"").toLowerCase()}function allowed(){var e=norm(location.hostname);return a.some(function(t){t=norm(t);return e===t||e.endsWith("."+t)})}function id(){if(window.crypto&&crypto.randomUUID)return crypto.randomUUID();return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(e){var t=Math.random()*16|0,r=e==="x"?t:t&3|8;return r.toString(16)})}function cookie(e,t,r){if(t===undefined){var o=document.cookie.match(new RegExp("(?:^|; )"+e.replace(/[.$?*|{}()[\\]\\\\/+^]/g,"\\\\$&")+"=([^;]*)"));return o?decodeURIComponent(o[1]):""}var i="";if(r){var l=new Date;l.setTime(l.getTime()+1e3*r);i="; expires="+l.toUTCString()}document.cookie=e+"="+encodeURIComponent(t)+i+"; path=/; SameSite=Lax"}function screenValue(){return Math.round((screen.width||0)*(devicePixelRatio||1))+"x"+Math.round((screen.height||0)*(devicePixelRatio||1))}function hash(e){var t=0;for(var r=0;r<e.length;r++)t=(Math.imul(31,t)+e.charCodeAt(r))|0;return Math.abs(t).toString(16)}function fp(){return"fp-"+hash([navigator.userAgent,navigator.language,screenValue(),new Date().getTimezoneOffset(),navigator.hardwareConcurrency||"",navigator.deviceMemory||""].join("|"))}function clean(e){return e==="#"||e==="/index.html"?"/":e}var started=Date.now(),eventId=id(),visitor="",session="";function keep(){visitor=cookie("elmas_test_v2_vid_"+w);if(!visitor){visitor=id();cookie("elmas_test_v2_vid_"+w,visitor,31536e3)}session=cookie("elmas_test_v2_sid_"+w);if(!session){session="session-"+id()}cookie("elmas_test_v2_sid_"+w,session,1800)}function payload(e,t){keep();var r=new URL(location.href);return{c:d,website:w,hostname:location.hostname,title:document.title||"",url:clean(r.pathname+r.search),href:r.href,referrer:clean(document.referrer||""),tag:"5",publishCode:p,fingerprint:fp(),ts:Date.now(),activeSeconds:Math.round((Date.now()-started)/1e3),pageViewEvent:eventId,name:e||"",newScript:"1",collectorNode:n,bundle:"inline-fallback",timestamp:Date.now(),screen:screenValue(),language:navigator.language||"",id:visitor,sesId:session,data:t||undefined}}function post(e,t){if(!allowed()||window[loaded]&&window[loaded][w])return;window[fallback]=window[fallback]||{};window[fallback][w]=true;fetch(c+(e?"/check":"/send"),{keepalive:true,method:"POST",body:JSON.stringify(payload(e,t)),headers:{"Content-Type":"application/json"},credentials:"omit"}).catch(function(){})}setTimeout(function(){post("",{source:"inline-fallback"});setInterval(function(){post("heartbeat",{reason:"inline-fallback"})},3e4)},1500)}();</script>`;
};
